	...
	EventFlow.prototype = {
		...
		spliceHtml :function(element,options){
			...
			for(var i = 0;i<options.events.length;i++){
				middleLineI +='';
				if((i+1)%2 == 0){
                       list += '<div class="list_right list" style="position:absolute;top:400px;">'
				}else{
				...
				for(var j = 0;j < options.events[i].events.length;j++){
					if((j+1)%2 == 0){
						list +='<div class="ev_text_odd ev_text ev_t'+options.events[i].events[j].mouths+'" id="yjss">'
					}else{
						list +='<div class="ev_text ev_text_event ev_t'+options.events[i].events[j].mouths+'" id="yjsj">'
					}
